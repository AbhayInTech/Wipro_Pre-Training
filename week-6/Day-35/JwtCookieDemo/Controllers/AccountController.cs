...

{
...

    {
        ...
        [HttpGet]
        public async Task<IActionResult> CookieLogin(string role, string name, string? empnumber = null)
...

    {
        ...
        new Claim("name", name), // user name

    };


            // Only add empnumber claim if it's provided
            if (!string.IsNullOrEmpty(empnumber))
            {
                claims.Add(new Claim("empnumber", empnumber)); // employee number
            }
